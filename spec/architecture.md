...
- The DataFrame returned by pandas' `.head()`/`.sample()` on the *original, unaggregated* frame is explicitly rejected (raises inside `summarize_for_llm`) if its row count equals the full dataset's row count and no aggregation function was detected in the executed code — this stops the trivial "just print the raw frame" case.
